...
for frame in range(
    ...
    floor_points = points_world[mask]

    if len(floor_points):
        points_all.append(floor_points)

    if frame % 500 == 0:
        print(
            f"Frame {frame}: "
            f"{len(floor_points):,} candidate points"
        )


if not points_all:
    raise RuntimeError("No candidate floor points found.")
...
if plane is None:
    raise RuntimeError(
        "Could not fit horizontal floor plane."
    )
